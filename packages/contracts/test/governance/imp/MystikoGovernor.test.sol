...
contract MystikoGovernorTest is Test, Random {
    ...
    function test_propose() public {
        ...
        bytes memory encodedError =
            abi.encodeWithSelector(IGovernor.GovernorInsufficientProposerVotes.selector, proposer, 0, proposalThreshold);
        vm.expectRevert(encodedError);
        vm.prank(proposer);
        ...
        encodedError = abi.encodeWithSelector(
            ...
            proposeId,
            governor.state(proposeId),
            _encodeStateBitmap(IGovernor.ProposalState.Succeeded) | _encodeStateBitmap(IGovernor.ProposalState.Queued)
        );
        vm.expectRevert(encodedError);
