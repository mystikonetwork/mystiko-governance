...
contract MystikoTimelockControllerTest is Test, Random {
    ...
    function _encodeStateBitmap(TimelockController.OperationState operationState) internal pure returns (bytes32) {
    ...
    function test_schedule_and_execute() public {
        ...
        bytes memory encodedError = abi.encodeWithSelector(
            IAccessControl.AccessControlUnauthorizedAccount.selector, account, controller.PROPOSER_ROLE()
        );
        ...
        encodedError = abi.encodeWithSelector(
            IAccessControl.AccessControlUnauthorizedAccount.selector, account, controller.EXECUTOR_ROLE()
        );
        vm.expectRevert(encodedError);
        vm.prank(account);
    ...
    function test_schedule_and_cancel() public {
        ...
        bytes memory encodedError = abi.encodeWithSelector(
            IAccessControl.AccessControlUnauthorizedAccount.selector, account, controller.CANCELLER_ROLE()
        );
        vm.expectRevert(encodedError);
        vm.prank(account);
    ...
    function test_update_delay() public {
        ...
        bytes memory encodedError =
            abi.encodeWithSelector(TimelockController.TimelockUnauthorizedCaller.selector, account);
        vm.expectRevert(encodedError);
        vm.prank(account);
